...
public class Weatherstation {
  public static void main(String[] args) throws FileNotFoundException, SeonsorNotAvailableException {
    ...
    SerialTemperatureDriver serialTemperatureSensor = new SerialTemperatureDriver(tempDevice);
    USBTemperatureDriver usbTemperatureDriver = new USBTemperatureDriver(usbTempDevice);
    SerialWindspeedDriver windSensor = new SerialWindspeedDriver(windDevice);
    SerialPressureSensor pressureSensor = new SerialPressureSensor(pressureDevice);
    SerialRainSensor rainSensor = new SerialRainSensor(rainDevice);

    Conversion conversion = new Conversion(serialTemperatureSensor, usbTemperatureDriver, windSensor, pressureSensor,
        rainSensor);

    System.out.println(Math.round(conversion.getTemperature()) + " degree(s) Celsius");
